# 灵枢 App 后端系统架构与快速参考

## 📋 项目结构

```
src/
├── index.ts                           # 主应用，路由集成
├── schema.ts                          # Drizzle ORM Schema 定义
├── auth/
│   └── index.ts                       # 认证模块 (登录注册、phone绑定)
├── member/
│   ├── quota.ts                       # 配额引擎 (会员判定、配额扣减、灵石兑换)
│   └── referral.ts                    # 分享奖励 (双向奖励闭环)
├── divination/
│   └── quota-check.ts                 # 排卦业务 (去重、配额检查、历史查询)
└── utils/
    ├── types.ts                       # TypeScript 类型定义
    └── response.ts                    # 响应工具与时间处理

drizzle/
├── 0000_moaning_susan_delgado.sql     # 初始迁移
└── 0001_extended_schema.sql           # Schema 扩展迁移
```

---

## 🔌 API 快速导航

### 认证接口

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/auth/login` | 登录即注册（username+password+phone） | ❌ |
| POST | `/auth/bind-phone` | 绑定手机号（触发分享闭环） | ✅ |

### 会员与配额接口

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/member/exchange` | 灵石兑换会员 (type: weekly\|monthly) | ✅ |
| GET | `/api/member/status` | 查询会员与配额状态 | ✅ |

### 分享推荐接口

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/referral/rewards` | 查询推荐奖励历史 | ✅ |

### 排卦业务接口

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/divination/check-quota` | 排卦前配额检查 (含去重) | ✅ |
| POST | `/api/divination/divine` | 执行排卦（消耗配额） | ✅ |
| GET | `/api/divination/history` | 查询排卦历史 | ✅ |

---

## 🔐 核心业务逻辑

### 1️⃣ 登录与注册（登录即注册）

**流程：**
```
用户输入 username + password + phone + 可选(referrerId, deviceId)
  ↓
检查 username 是否存在？
  ├─ 不存在 → 创建新用户
  │  ├─ dailyFreeQuota = 1
  │  ├─ bonusQuota = 2（首日赠送）
  │  ├─ lingshi = 0
  │  └─ memberLevel = 0
  │
  └─ 存在 → 验证 password + phone 是否匹配
     └─ 匹配 → 返回用户信息

签发 JWT Token（30天有效期）
```

### 2️⃣ 配额管理（优先级顺序）

**排卦前检查优先级：**
1. **会员免费** → 若 memberLevel=1 且 memberExpireAt > now，无需扣费，直接放行
2. **每日免费配额** → 若 lastUsedDate != 今天，重置 dailyFreeQuota=1；若 >0，可用
3. **赠送配额** → 若 bonusQuota > 0，可用；扣减时使用 WHERE bonusQuota > 0 防止负数
4. **灵石兑换** → 若都用完，提示用户灵石不足，建议兑换

**灵石兑换价格：**
```
700 灵石  = 7 天周会员
3000 灵石 = 30 天月会员
```

**会员有效期堆叠逻辑：**
```javascript
newExpireAt = max(now, currentMemberExpireAt) + daysToAdd
// 确保重复兑换时有效期顺延而非覆盖
```

### 3️⃣ 分享奖励闭环

**触发条件：** 新用户同时完成
- ✅ username 注册
- ✅ phone 绑定

**双向奖励：**
```
新用户奖励：
  └─ 已在注册时获得 3 次体验（1 dailyFreeQuota + 2 bonusQuota）

老用户（推荐人）奖励：
  ├─ 100 灵石（无上限）
  └─ bonus_quota +1（上限5，达到上限后仅增加灵石）
```

### 4️⃣ 排卦去重与配额扣减

**5 分钟去重哈希：**
```
SHA256(userId + subject + category)
// 同一用户，同一事由，同一类别，5分钟内不重复扣费
```

**配额检查顺序：**
```
POST /api/divination/check-quota
  ↓
计算 subjectHash
  ↓
检查 5 分钟内是否有相同 hash 的记录？
  ├─ 是 → isDuplicate=true，不可排卦
  └─ 否 → 检查配额状态
         ├─ 会员状态？→ canDivine=true
         ├─ 免费次数？→ canDivine=true
         ├─ 赠送次数？→ canDivine=true
         └─ 都没有？→ canDivine=false
```

---

## 🗂️ 数据库表结构

### users 表
```typescript
{
  id: integer (PK),
  username: text (UNIQUE),                 // 登录账号
  phone: text (UNIQUE),                    // 手机号
  password: text,                          // 加密密码
  dailyFreeQuota: integer (默认1),         // 每日免费次数
  bonusQuota: integer (默认0),             // 赠送次数，上限5
  lastUsedDate: integer (默认0),           // 上次使用日期 YYYYMMDD
  lingshi: integer (默认0),                // 灵石余额
  memberLevel: integer (默认0),            // 0=非会员, 1=会员
  memberExpireAt: integer (默认0),         // 会员过期时间戳
  referrerId: integer (FK),                // 推荐人ID
  deviceId: text,                          // 设备指纹（风控）
  createdAt: integer,                      // 创建时间戳
  updatedAt: integer,                      // 更新时间戳
}
```

### divinationRecords 表
```typescript
{
  id: integer (PK),
  userId: integer (FK),                    // 用户ID
  subjectHash: text (INDEX),               // SHA256 去重哈希
  inputData: text,                         // 脱敏的起卦参数 (JSON)
  lastUsedAt: integer,                     // 最后使用时间戳
  createdAt: integer,                      // 记录创建时间戳
}
```

### referralRewards 表
```typescript
{
  id: integer (PK),
  referrerId: integer (FK),                // 推荐人ID
  inviteeId: integer (FK),                 // 被推荐人ID
  rewardType: text,                        // 'lingshi' | 'bonus_quota'
  lingshiAwarded: integer (默认0),         // 灵石奖励数量
  bonusQuotaAwarded: integer (默认0),      // 配额奖励数量
  createdAt: integer,                      // 奖励时间戳
}
```

---

## 🔑 关键特性

### 并发安全性
```typescript
// ✅ 防止负值扣减
UPDATE users
SET bonus_quota = bonus_quota - 1
WHERE id = ? AND bonus_quota > 0  // 关键条件

// ✅ 会员有效期堆叠
const newExpireAt = Math.max(now, user.memberExpireAt) + daysToAdd;
```

### 时间处理
```typescript
// 统一使用 UTC+8（北京时间）
function getTodayDate(): number {
  // 返回 YYYYMMDD 格式的数字，如 20260204
  const utc8 = new Date(now.getTime() + 8 * 60 * 60 * 1000);
  return parseInt(`${year}${month}${date}`);
}
```

### 隐敏处理
```typescript
// 手机号隐敏：189****1234
maskPhone("18900001234") → "189****1234"
```

---

## 📊 响应格式统一

### 成功响应 (200)
```json
{
  "code": 200,
  "message": "操作成功",
  "data": { /* 业务数据 */ }
}
```

### 错误响应
```json
{
  "code": 400 | 401 | 403 | 500,
  "message": "错误描述信息"
}
```

---

## 🧪 常见测试场景

### 场景 1：完整新用户流程
```bash
1. POST /auth/login (username + password + phone)
   ↓ 获得 token
2. GET /api/member/status
   ↓ 查看初始配额 (1 dailyFreeQuota + 2 bonusQuota)
3. POST /api/divination/divine
   ↓ 排卦并扣减 1 次配额
4. GET /api/member/status
   ↓ 配额变为 (0 dailyFreeQuota + 2 bonusQuota)
```

### 场景 2：推荐奖励闭环
```bash
1. 推荐人创建账户 → 记录 ID (如 5)
2. 新用户登录并携带 referrerId=5
3. 新用户执行 POST /auth/bind-phone
   ↓ 自动触发分享闭环
4. 推荐人查询 GET /api/referral/rewards
   ↓ 显示 +100 灵石 + 1 bonus_quota
```

### 场景 3：灵石兑换会员
```bash
1. 用户拥有 700+ 灵石
2. POST /api/member/exchange (type: weekly)
   ↓ 扣减 700 灵石，memberExpireAt 顺延 7 天
3. 在会员有效期内排卦无需扣费
```

### 场景 4：5分钟去重
```bash
1. POST /api/divination/divine (subject: "财运", category: "财运")
   ↓ 成功排卦，记录 hash
2. 立即再请求相同内容
   ↓ isDuplicate=true，不扣费，返回错误
3. 等待 5 分钟后重试
   ↓ 成功排卦
```

---

## 🚀 本地开发与部署

### 本地开发
```bash
# 安装依赖
npm install

# 启动开发服务器
npm run dev
# → http://localhost:8787

# 应用迁移
npx drizzle-kit push:sqlite

# 查看数据库
npx drizzle-kit studio
```

### 生产部署 (Cloudflare Workers)
```bash
# 设置环境变量（wrangler.toml）
JWT_SECRET = "强随机密钥"

# 部署
npm run deploy
```

---

## ⚠️ 常见问题

### Q1: 为什么新用户有 2 次 bonusQuota？
A: 新用户首日体验策略 → 1 dailyFreeQuota + 2 bonusQuota = 3 次，提高用户首日体验。

### Q2: 会员过期后，配额如何重置？
A: 会员过期后，下一次排卦时检查 memberExpireAt，发现已过期则按优先级检查免费配额。

### Q3: 同一用户，不同事由，5分钟内是否会去重？
A: **不会**。Hash 包含了 subject 和 category，不同的问题有不同的 hash，不会被视为重复。

### Q4: bonusQuota 超过 5 上限后怎么办？
A: 达到上限（5）后，新分享成功不再增加 bonusQuota，仅增加 100 灵石。

### Q5: 用户可以一次兑换多个会员吗？
A: 可以。每次兑换时，新有效期 = max(now, 当前有效期) + 兑换天数，实现堆叠。

---

## 📞 技术栈

- **框架：** Hono.js (Cloudflare Workers)
- **数据库：** SQLite via D1
- **ORM：** Drizzle ORM
- **认证：** JWT (HS256)
- **加密：** bcryptjs
- **时间处理：** UTC+8 (北京时间)

---

## 📝 版本记录

| 版本 | 日期 | 描述 |
|------|------|------|
| 1.0.0 | 2026-02-04 | 初始版本：登录注册、配额管理、分享奖励、排卦去重 |

---

Generated with ❤️ for 灵枢 App Backend
